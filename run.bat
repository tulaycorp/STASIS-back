@echo off
title STASIS Application Launcher
color 0A

echo ===============================================
echo    STASIS Application Launcher
...
echo [1/3] Starting Spring Boot Backend with Live Data (Supabase)...
echo       Profile: supabase
echo       Port: 8080
echo       Actuator endpoints: http://localhost:8080/actuator
echo.
start "STASIS Backend - Live Data" cmd /k "echo Starting Spring Boot with Supabase (Live Data)... && %MVN_CMD% spring-boot:run -Dspring-boot.run.profiles=supabase"
...
echo.
echo Database: Connected to Supabase (Live Data)
echo.
